...
luaunit.VERSION='3.2'
luaunit._VERSION= luaunit.VERSION -- For LuaUnit v2 compatibility

--[[ Some people like assertEquals( actual, expected ) and some people prefer
assertEquals( expected, actual ).
]]--
luaunit.ORDER_ACTUAL_EXPECTED = true
luaunit.PRINT_TABLE_REF_IN_ERROR_MSG = false
luaunit.TABLE_EQUALS_KEYBYCONTENT = true
luaunit.LINE_LENGTH=80

-- set this to false to debug luaunit
local STRIP_LUAUNIT_FROM_STACKTRACE=true

luaunit.VERBOSITY_DEFAULT = 10
luaunit.VERBOSITY_LOW     = 1
luaunit.VERBOSITY_QUIET   = 0
luaunit.VERBOSITY_VERBOSE = 20
...
luaunit.FAILURE_PREFIX = 'LuaUnit test FAILURE: ' -- prefix string for failed tests

luaunit.USAGE=[[Usage: lua <your_test_suite.lua> [options] [testname1 [testname2] ... ]
Options:
...
luaunit.private.__genSortedIndex = __genSortedIndex
...
luaunit.private.sortedPairs = sortedPairs
...
luaunit.private.strMatch = strMatch
...
luaunit.private.xmlEscape = xmlEscape

local function xmlCDataEscape( s )
    -- Return s escaped for CData section, escapes: "]]>"
    return string.gsub( s, ']]>', ']]&gt;' )
end
luaunit.private.xmlCDataEscape = xmlCDataEscape
...
luaunit.private.stripLuaunitTrace = stripLuaunitTrace
...
local function prettystr_sub(v, indentLevel, keeponeline, printTableRefs, recursionTable )
    ...
    elseif "table" == type_v then
        ...
        --end
        return luaunit.private._table_tostring(v, indentLevel, printTableRefs, recursionTable)
    end
...
local function prettystr( v, keeponeline )
    ...
    * tables are expanded
    ...
    local s = prettystr_sub(v, 1, keeponeline, luaunit.PRINT_TABLE_REF_IN_ERROR_MSG, recursionTable)
    if recursionTable.recursionDetected and not luaunit.PRINT_TABLE_REF_IN_ERROR_MSG then
...
luaunit.private.prettystrPadded = prettystrPadded
...
luaunit.private._table_keytostring = _table_keytostring
...
local function _table_tostring( tbl, indentLevel, printTableRefs, recursionTable )
    printTableRefs = printTableRefs or luaunit.PRINT_TABLE_REF_IN_ERROR_MSG
    ...
    for k, v in ipairs( result ) do
        ...
        if totalLength >= luaunit.LINE_LENGTH then
    ...
    if not dispOnMultLines then
        ...
        dispOnMultLines = totalLength + 2 >= luaunit.LINE_LENGTH
    end
...
luaunit.private._table_tostring = _table_tostring -- prettystr_sub() needs it
        ...
        for _, value in pairs(t) do
            if type(value) == type_e then
                ...
                if type_e == 'table' then
                    ...
                    -- items is sufficient
                    if luaunit.private._is_table_equals( value, element ) then
                        return true
                    end
                end
            end
...
local function _is_table_equals(actual, expected, recursions)
    ...
    if (type_a == 'table') --[[ and (type_e == 'table') ]] and not recursions[actual] then
        ...
        for k, v in pairs(actual) do
            if luaunit.TABLE_EQUALS_KEYBYCONTENT and type(k) == "table" then
        ...
        if luaunit.TABLE_EQUALS_KEYBYCONTENT then
...
luaunit.private.fail_fmt = fail_fmt
...
local function errorMsgEquality(actual, expected)
    if not luaunit.ORDER_ACTUAL_EXPECTED then
...
function luaunit.assertError(f, ...)
...
function luaunit.assertTrue(value)
    if not value then
        failure("expected: true, actual: " ..prettystr(value), 2)
    end
end

function luaunit.assertFalse(value)
    if value then
        failure("expected: false, actual: " ..prettystr(value), 2)
    end
end

function luaunit.assertIsNil(value)
    if value ~= nil then
        failure("expected: nil, actual: " ..prettystr(value), 2)
    end
end

function luaunit.assertNotIsNil(value)
    if value == nil then
...
local EPSILON = 1E-11
function luaunit.almostEquals(actual, expected, margin, margin_boost )
...
function luaunit.assertAlmostEquals(actual, expected, margin )
    -- check that two floats are close by margin
    if not luaunit.almostEquals(actual, expected, margin) then
        if not luaunit.ORDER_ACTUAL_EXPECTED then
            expected, actual = actual, expected
        end
...
function luaunit.assertNotAlmostEquals(actual, expected, margin )
    -- check that two floats are not close by margin
    if luaunit.almostEquals(actual, expected, margin) then
        if not luaunit.ORDER_ACTUAL_EXPECTED then
            expected, actual = actual, expected
        end
...
function luaunit.assertStrContains(str, sub, useRe )
...
function luaunit.assertStrIContains(str, sub )
    -- this relies on lua string.find function
    -- a string always contains the empty string
...
function luaunit.assertNotStrContains(str, sub, useRe )
    -- this relies on lua string.find function
    -- a string always contains the empty string
...
function luaunit.assertNotStrIContains(str, sub )
    -- this relies on lua string.find function
    -- a string always contains the empty string
...
function luaunit.assertStrMatches(str, pattern, start, final )
...
function luaunit.assertErrorMsgEquals(expectedMsg, func, ... )
    -- assert that calling f with the arguments will raise an error
    -- example: assertError( f, 1, 2 ) => f(1,2) should generate an error
...
function luaunit.assertErrorMsgContains(partialMsg, func, ... )
    -- assert that calling f with the arguments will raise an error
    -- example: assertError( f, 1, 2 ) => f(1,2) should generate an error
    local no_error, error_msg = pcall( func, ... )
    if no_error then
...
function luaunit.assertErrorMsgMatches(expectedMsg, func, ... )
    -- assert that calling f with the arguments will raise an error
    -- example: assertError( f, 1, 2 ) => f(1,2) should generate an error
    local no_error, error_msg = pcall( func, ... )
    if no_error then
...
for _, funcName in ipairs(
    ...
    luaunit[funcName] = function(value)
...
for _, typeExpected in ipairs(
    ...
    luaunit['is'..typeExpected] = isType
    luaunit['is_'..typeExpectedLower] = isType
end
...
function luaunit.assertIs(actual, expected)
    if actual ~= expected then
        if not luaunit.ORDER_ACTUAL_EXPECTED then
...
function luaunit.assertNotIs(actual, expected)
    if actual == expected then
        if not luaunit.ORDER_ACTUAL_EXPECTED then
            expected = actual
        end
...
function luaunit.assertItemsEquals(actual, expected)
...
for _,v in ipairs( list_of_funcs ) do
    ...
    luaunit[alias] = luaunit[funcname]

    if EXPORT_ASSERT_TO_GLOBALS then
        _G[funcname] = luaunit[funcname]
        _G[alias] = luaunit[funcname]
    end
end
...
luaunit.genericOutput = genericOutput -- publish, so that custom classes may derive from it
...
function TapOutput.new(runner)
    local t = genericOutput.new(runner, luaunit.VERBOSITY_LOW)
...
function TextOutput.new(runner)
    local t = genericOutput.new(runner, luaunit.VERBOSITY_DEFAULT)
    t.errorList = {}
...
function luaunit.LuaUnit.isMethodTestName(s )
...
function luaunit.LuaUnit.parseCmdLine(cmdLine )
    ...
    local function parseOption( option )
        ...
        elseif option == '--verbose' or option == '-v' then
            result['verbosity'] = luaunit.VERBOSITY_VERBOSE
            return
        elseif option == '--quiet' or option == '-q' then
            result['verbosity'] = luaunit.VERBOSITY_QUIET
            return
    ...
    if result['help'] then
        luaunit.LuaUnit.help()
    end

    if result['version'] then
        luaunit.LuaUnit.version()
    end
...
function luaunit.LuaUnit.help()
    print(luaunit.USAGE)
    os.exit(0)
end

function luaunit.LuaUnit.version()
    print('LuaUnit v'.. luaunit.VERSION..' by Philippe Fremy <phil@freehackers.org>')
    os.exit(0)
end

function luaunit.LuaUnit.patternInclude(patternFilter, expr )
...
function luaunit.LuaUnit.statusLine(result)
...
function luaunit.LuaUnit:startSuite(testCount, nonSelectedCount)
    self.result = {
        testCount = testCount,
...
function luaunit.LuaUnit:startClass(className )
    self.result.currentClassName = className
    self.output:startClass( className )
end

function luaunit.LuaUnit:startTest(testName  )
...
function luaunit.LuaUnit:addStatus(err )
...
function luaunit.LuaUnit:endTest()
    local node = self.result.currentNode
...
function luaunit.LuaUnit:endClass()
    self.output:endClass()
end

function luaunit.LuaUnit:endSuite()
...
function luaunit.LuaUnit:setOutputType(outputType)
    -- default to text
...
function luaunit.LuaUnit:protectedCall(classInstance, methodInstance, prettyFuncName)
    ...
    local failed
    err.msg, failed = err.msg:gsub(luaunit.FAILURE_PREFIX, "", 1)
    if failed > 0 then
...
function luaunit.LuaUnit:execOneFunction(className, methodName, classInstance, methodInstance)
...
function luaunit.LuaUnit.expandOneClass(result, className, classInstance )
    -- add all test methods of classInstance to result
    for methodName, methodInstance in sortedPairs(classInstance) do
        if luaunit.LuaUnit.asFunction(methodInstance) and luaunit.LuaUnit.isMethodTestName( methodName ) then
            table.insert( result, { className..'.'..methodName, classInstance } )
        end
    end
end

function luaunit.LuaUnit.expandClasses(listOfNameAndInst )
    ...
    for i,v in ipairs( listOfNameAndInst ) do
        ...
        if luaunit.LuaUnit.asFunction(instance) then
            ...
            if luaunit.LuaUnit.isClassMethod( name ) then
                local className, methodName = luaunit.LuaUnit.splitClassMethod( name )
                ...
                luaunit.LuaUnit.expandOneClass( result, name, instance )
            end
        end
    end
...
function luaunit.LuaUnit.applyPatternFilter(patternFilter, listOfNameAndInst )
    local included, excluded = {}, {}
    for i, v in ipairs( listOfNameAndInst ) do
        -- local name, instance = v[1], v[2]
        if luaunit.LuaUnit.patternInclude( patternFilter, v[1] ) then
...
function luaunit.LuaUnit:runSuiteByInstances(listOfNameAndInst )
    ...
    for i,v in ipairs( filteredList ) do
        ...
        if luaunit.LuaUnit.asFunction(instance) then
            ...
            if type(instance) ~= 'table' then
                ...
                assert( luaunit.LuaUnit.isClassMethod( name ) )
                local className, methodName = luaunit.LuaUnit.splitClassMethod( name )
                local methodInstance = instance[methodName]
                if methodInstance == nil then
                    error( "Could not find method in class "..tostring(className).." for method "..tostring(methodName) )
...
function luaunit.LuaUnit:runSuiteByNames(listOfName )
...
function luaunit.LuaUnit:runSuite( ... )
    ...
    local no_error, val = pcall( luaunit.LuaUnit.parseCmdLine, args )
    ...
    self:runSuiteByNames( options.testNames or luaunit.LuaUnit.collectTests() )
...
function luaunit:setVerbosity(verbosity )
    luaunit.LuaUnit.verbosity = verbosity
end
